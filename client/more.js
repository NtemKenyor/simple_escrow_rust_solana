...
class EscrowState {
    constructor(fields = {}) {
        ...
        this.amount = fields.amount || new BN(0);
        this.buyer_approved = fields.buyer_approved || 0;
        this.seller_approved = fields.seller_approved || 0;
    }
}
...
async function initializeEscrow(connection, programId, payer, sellerPubkey, amount) {
    ...
    const escrowState = new EscrowState({
        buyer_pubkey: payer.publicKey.toBytes(),
        seller_pubkey: sellerPubkey.toBytes(),
        amount: new BN(amount), // Ensure amount is a BN instance
        buyer_approved: 0,
        seller_approved: 0,
    });

    // Serialize the escrow state
    const serializedEscrowState = borsh.serialize(EscrowStateSchema, escrowState);
